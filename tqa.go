...
var tqaPath = home + "/.tqa"
var gptStore = store.InitStore(tqaPath)
...
func actionFlags(f *flag.FlagSet) {
	if name := f.Arg(0); name != "" {
		...
		case "k":
			...
			if err := store.SetOpenAiApiKey(tqaPath, apiKey); err != nil {
				fmt.Println(err)
				return
			}
		case "e":
			filename, err := store.GetLatestContent(tqaPath)
			if err != nil {
				fmt.Println("File export failed")
				return
			}
			utils.DownloadFile(tqaPath + "/content/" + filename)
...
func processQA() {
	config, err := store.GetConfig(tqaPath)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	for {
		select {
		case <-quitChan:
			store.UpdateContent(tqaPath, msg)
			return
		case input := <-iptChan:
